...
public boolean canGo(int i, int j, Spaceoccupier[][] board) {
    if ((((i - this.i <= 1) && (i - this.i >= -1)) &&
         ((j - this.j <= 1) && (j - this.j >= -1))) &&
        ((i - this.i != 0) || (j - this.j != 0))) {
        return !finalHouseIsBlocked(i, j, board);
    }
    return false;
}
    ...
    public boolean isValidMove(int i, int j, Spaceoccupier[][] board) {
        return canGo(i, j, board) && isValidAboutCheck(i, j, board) && !finalHouseIsBlocked(i, j, board);
